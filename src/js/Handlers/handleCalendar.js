...
import elementFactory from '../../BuildElement/BuildElement'
...
const populateEventTemplateDetails = monthEvent => {
  const eventDetailsContent = EventTemplate.details.map((detail, index) => {
    detail.content = monthEvent[index];
    return detail;
  });
  return eventDetailsContent;
};
const getEvents = eventsToGet => eventsToGet || utilities.getFile(`${eventsCalendar.eventsFilePath}${utilities.getMonthName()}.js`);

const handleCalendar = eventsToGet => {
  if (handleValidation(getEvents(eventsToGet))) {
    const monthEvents = utilities.parseFile(getEvents(eventsToGet).responseText);
    monthEvents.forEach(monthEvent => {
      builder(populateEventTemplateDetails(Object.keys(monthEvent).map(function (e) { return monthEvent[e] })), builder(EventTemplate.detailsWrap, builder(EventTemplate.wrap, document.querySelector('.eventsCalendar__events'))));
